...
void ADistrictsCharacter::BeginPlay()
{
	// Call the base class  
	Super::BeginPlay();
...
void ADistrictsCharacter::SetupPlayerInputComponent(class UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("ResetVR", IE_Pressed, this, &ADistrictsCharacter::OnResetVR);

	// Bind movement events
	PlayerInputComponent->BindAxis("MoveForward", this, &ADistrictsCharacter::MoveForward);
	PlayerInputComponent->BindAxis("MoveRight", this, &ADistrictsCharacter::MoveRight);
	...
	if (ProjectileClass != NULL)
	{
		...
		if (World != NULL)
		{
			if (bUsingMotionControllers)
			{
				...
				World->SpawnActor<ADistrictsProjectile>(ProjectileClass, SpawnLocation, SpawnRotation);
			}
			else
			{
				...
				World->SpawnActor<ADistrictsProjectile>(ProjectileClass, SpawnLocation, SpawnRotation, ActorSpawnParams);
			}
		}
	}
...
void ADistrictsCharacter::OnResetVR()
{
	UHeadMountedDisplayFunctionLibrary::ResetOrientationAndPosition();
}

void ADistrictsCharacter::BeginTouch(const ETouchIndex::Type FingerIndex, const FVector Location)
{
...
void ADistrictsCharacter::EndTouch(const ETouchIndex::Type FingerIndex, const FVector Location)
{
...
//void ADistrictsCharacter::TouchUpdate(const ETouchIndex::Type FingerIndex, const FVector Location)
//{
...
void ADistrictsCharacter::MoveForward(float Value)
{
	if (Value != 0.0f)
	{
...
void ADistrictsCharacter::MoveRight(float Value)
{
	if (Value != 0.0f)
	{
		// add movement in that direction
...
void ADistrictsCharacter::TurnAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
	AddControllerYawInput(Rate * BaseTurnRate * GetWorld()->GetDeltaSeconds());
}

void ADistrictsCharacter::LookUpAtRate(float Rate)
{
	// calculate delta for this frame from the rate information
...
bool ADistrictsCharacter::EnableTouchscreenMovement(class UInputComponent* PlayerInputComponent)
{
	...
	{
		PlayerInputComponent->BindTouch(EInputEvent::IE_Pressed, this, &ADistrictsCharacter::BeginTouch);
		PlayerInputComponent->BindTouch(EInputEvent::IE_Released, this, &ADistrictsCharacter::EndTouch);

		//Commenting this out to be more consistent with FPS BP template.
		//PlayerInputComponent->BindTouch(EInputEvent::IE_Repeat, this, &ADistrictsCharacter::TouchUpdate);
		return true;
	}
